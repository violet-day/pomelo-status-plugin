...
StatusManager.prototype.onlineUser = function (pattern, cb) {
  pattern == pattern || '*';
  var self = this;
  this.redis.keys(pattern, function (err, list) {
    if (err) {
      return cb(err);
    } else {
      cb(null, list.map(function (uid) {
        return uid.replace(self.prefix + ':')
      }))
    }
  });
};
